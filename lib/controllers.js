...
export const errorInternalController = (error, request, response, _next) => {
...
export const projectController = (request, response) => {
  const { slug } = request.params;
...
export const appsController = (services) => {
  return (request, response) => {
    services = services.filter((service) => service.platform === "app");

    return response.render("apps.njk", { services });
  };
};

export const viewController = (request, response) => {
